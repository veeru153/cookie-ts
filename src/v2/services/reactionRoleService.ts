...
export const reactionRoleService = async (reaction: MessageReaction, user: User, action: string) => {
    ...
    logger.info(`[Reaction Role Service] ${username}#${discriminator} (${id}) ${actionStr} '${reaction.emoji.name}' on Message : ${message.id}`);
    ...
    switch (action) {
        ...
        default:
            logger.error(`[Reaction Role Service] ${Errors.ROLE_REACTION_UNKNOWN_ACTION}`);
    }
}
